...
% number of decimal places that delta is rounded to 
%-------
deltanum=delta;
afterdecimal=  char( extractAfter( string(deltanum),'.') );
dec_places_delta=length(afterdecimal);
%-------
       ...
       if randn<0                                                           % if randn<0 mutate in mass, else mutate in \alpha.
                ...
                mgenotypes(length(mgenotypes)+1)=round(mgenotypes(q+1)+delta*sign(randn),dec_places_delta);
                ...
                alphagenotypes(length(alphagenotypes)+1)=round(alphagenotypes(q+1)+delta*sign(randn),dec_places_delta);

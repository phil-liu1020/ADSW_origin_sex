function [genotypeNEVOL_m,genotypeNEVOL_alpha,m,alpha,m2,alpha2]=Evolutionary_trajectories_1realisation(m0,alpha0,A,M,T,Cz,Cp,beta1,beta2,lambda21,lambda12,mu,NEVOL,f0,delta,alphamax, switching_environments, return_genotypes,plasticity)
...
random2=geornd(   (mu+lambda21)*ones(1,9000000)      )+ones(1,9000000) ;
random1=geornd(   (mu+lambda12)*ones(1,9000000)      )+ones(1,9000000) ;

genotypeNEVOL_m=cell(NEVOL,2);
genotypeNEVOL_alpha=cell(NEVOL,2);
...
while Nmut<NEVOL
 ...
 if Env2==1
   ...
   if rand<mu/(mu+lambda21)
       f=f(:); f2=f2(:);
       [mgenotypes,alphagenotypes,mgenotypes2,alphagenotypes2,f,f2]=Introduce_Mutant_Plasticity(mgenotypes,mgenotypes2,alphagenotypes,alphagenotypes2,f,f2,f0,A,M,T,Cz,Cp,beta1,beta2,tnext,delta,plasticity,alphamax,Env1,Env2);
       ...
       f=Invasion_dynamics(mgenotypes,alphagenotypes,S,beta2,Cz,Cp,tnext,T,A,M,f);
       end
...
Nev=Nev+1;
   ...
   if rand<mu/(mu+lambda12)
       ...
       if plasticity==1
       f=Invasion_dynamics(mgenotypes,alphagenotypes,S,beta1,Cz,Cp,tnext,T,A,M,f);
       else
       f=Invasion_dynamics(mgenotypes,alphagenotypes,S,beta1,Cz,Cp,tnext,T,A,M,f);
       end
